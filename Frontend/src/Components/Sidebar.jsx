...
export function Sidebar() {
  ...
  const menuItems = [
    ...
    { icon: <FaTasks size={20} />, label: 'Tasks' },
    { icon: <FaProjectDiagram size={20} />, label: 'Projects' },
  ...
  return (
    <nav
      className={`shadow-md bg-gray-800 p-2 flex flex-col transition-all duration-300
        ${open ? 'w-60' : 'w-20'} 
        h-[calc(100vh-64px)] overflow-y-auto`}  // 👈 64px = navbar height
    >
      {/* Header */}
      ...
      <ul className="flex-grow">
        {menuItems.map((item, index) => (
          <li
            key={index}
            className="group relative px-3 py-2 hover:bg-blue-700 rounded-md duration-300 cursor-pointer flex gap-3"
          >
            <div>{item.icon}</div>
            ...
            {!open && (
              <span className="absolute left-12 text-white bg-gray-900 rounded-md px-4 py-1 shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {item.label}
              </span>
            )}
          </li>
