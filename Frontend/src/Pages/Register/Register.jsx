...
export default function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const data = await register(email, password, mode);
            loginUser(data); // the AuthContext handles storing token, decoding it, and setting user & mode
        ...
        <div className="flex items-center p-8 justify-center min-h-screen bg-gray-800">
            <div className="bg-gray-800 backdrop-blur-md border border-white/600 p-8 rounded-2xl shadow-lg w-full max-w-md ">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <button type="submit" className="w-full bg-white text-purple-600 font-semibold  py-3 rounded-xl hover:bg-white/90 hover:shadow-xl hover:scale-105 transition-all duration-300 mt-6">Register</button>
                </form>
            </div></div>
    )
}